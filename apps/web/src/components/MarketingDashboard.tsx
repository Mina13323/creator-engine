...
export default function MarketingDashboard() {
  ...
  const getPlatformIcon = (platform: string) => {
    ...
    if (p.includes('facebook')) return <Facebook className="w-4 h-4 text-emerald-500" />;
  ...
  return (
    <div className="min-h-screen pb-20 relative">
      ...
      >
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-[#2e403d] rounded-2xl p-6 text-white flex flex-col md:flex-row items-center justify-between shadow-lg">
          <div>
            <h2 className="text-xl font-bold mb-1 flex items-center gap-2">
              <Megaphone className="w-5 h-5" /> Marketing Strategies & Go-To-Market
            </h2>
            <p className="text-emerald-100 text-sm">Your AI-generated marketing playbook is ready. Tap any card to read the full content.</p>
          </div>
          <div className="flex items-center gap-3 mt-4 md:mt-0">
            ...
            <button className="px-4 py-2 bg-white text-emerald-600 hover:bg-emerald-50 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center gap-2">
          ...
          <div className="xl:col-span-1 space-y-8">
            <Card className="p-6 border-slate-200 shadow-sm rounded-3xl bg-white hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                  <Target className="w-5 h-5 text-emerald-500" /> Core Strategy
                </h3>
              </div>
            ...
            <Card className="p-6 border-slate-200 shadow-sm rounded-3xl bg-slate-900 text-white overflow-hidden relative">
              ...
              <h3 className="text-lg font-bold mb-6 flex items-center gap-2 relative z-10">
                <Rocket className="w-5 h-5 text-emerald-400" /> Launch Playbook
              </h3>
              {marketingCampaign.launchPlan && typeof marketingCampaign.launchPlan === 'object' ? (
                <div className="relative border-l border-emerald-500/30 ml-3 pl-5 space-y-6 z-10">
                  {Object.entries(marketingCampaign.launchPlan).map(([phase, details], idx) => (
                    <div key={idx} className="relative group">
                      <div className="absolute -left-[27px] top-1.5 w-3 h-3 rounded-full bg-emerald-500 ring-4 ring-slate-900"></div>
                      <div className="space-y-1 cursor-pointer hover:bg-slate-800/50 p-2 -ml-2 rounded-lg transition-colors" onClick={() => setSelectedItem({ type: 'campaign', data: { name: phase, body: details }})}>
                        <h4 className="text-sm font-bold text-emerald-300">{phase}</h4>
                    ...
                    <>
                      ...
                      {selectedItem.data.goal && (
                        <div>
                          ...
                          <p className="text-slate-600 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100/50">{selectedItem.data.goal}</p>
                        </div>
                      )}
                    ...
                    <>
                      <div>
                        <h4 className="text-sm font-bold text-slate-900 mb-2 flex justify-between">
                          Body Copy
                          <button onClick={() => copyToClipboard(selectedItem.data.body, 1)} className="text-emerald-600 hover:text-emerald-700 text-xs flex items-center gap-1">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-slate-900 mb-2 flex justify-between">
                          Call To Action
                          <button onClick={() => copyToClipboard(selectedItem.data.callToAction, 2)} className="text-emerald-600 hover:text-emerald-700 text-xs flex items-center gap-1">
                      ...
                      {selectedItem.data.hook && (
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-2 flex justify-between">
                            Hook Idea
                            <button onClick={() => copyToClipboard(selectedItem.data.hook, 3)} className="text-emerald-600 hover:text-emerald-700 text-xs flex items-center gap-1">
